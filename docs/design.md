...
UPDATED: 19th October 2016 - **additions** ~~deletions~~
...
A menu on the left hand side provides the areas of the dashboard that are available to the user:
...
The number of columns will adapt depending on screen size from a maximum of 2 down to 1 for mobiles.
...
The current proposal is for histograms for: age, BMI, usage per hour, and prescribed exercise frequency. Also a pie chart for sex.
